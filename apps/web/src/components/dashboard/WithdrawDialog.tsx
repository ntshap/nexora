import { FormEvent, useEffect, useState } from "react";
import type { Address } from "viem";

import { Button } from "@/components/ui/button";
import { useVaultTransactions } from "@/hooks/use-vault-transactions";

type WithdrawDialogProps = {
  isOpen: boolean;
  onClose: () => void;
  vaultAddress: Address;
  defaultReceiver?: Address;
  vaultLabel?: string;
  assetSymbol?: string;
  onSuccess?: () => Promise<void> | void;
};

export const WithdrawDialog = ({
  isOpen,
  onClose,
  vaultAddress,
  defaultReceiver,
  vaultLabel = "SynthVault",
  assetSymbol = "mUSDC",
  onSuccess,
}: WithdrawDialogProps) => {
  const [amount, setAmount] = useState("");
  const [receiver, setReceiver] = useState<Address | "">("");
  const [message, setMessage] = useState<string | null>(null);

  const { executeWithdraw, isWithdrawPending, isConnected } = useVaultTransactions({
    vaultAddress,
    onComplete: onSuccess,
  });

  useEffect(() => {
    if (isOpen) {
      setMessage(null);
      setAmount("");
      setReceiver(defaultReceiver ?? "");
    }
  }, [defaultReceiver, isOpen]);

  if (!isOpen) {
    return null;
  }

  const submit = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    try {
      if (!receiver) {
        throw new Error("Enter a receiving address");
      }
      const status = await executeWithdraw(amount, {
        vaultLabel,
        assetSymbol,
        receiver: receiver as Address,
      });
      setMessage(status);
      setAmount("");
    } catch (error) {
      setMessage((error as Error).message);
    }
  };

  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/70 px-4 backdrop-blur-md">
      <div className="w-full max-w-md rounded-3xl border border-white/10 bg-[#101123] p-6 shadow-hero">
        <header className="mb-4 space-y-1">
          <h2 className="text-2xl font-manrope font-semibold text-hero-text">Send from {vaultLabel}</h2>
          <p className="text-sm text-hero-text-muted">Withdraw {assetSymbol} to another wallet.</p>
        </header>
        <form onSubmit={submit} className="grid gap-4">
          <input
            type="number"
            min="0"
            step="0.01"
            value={amount}
            onChange={(event) => setAmount(event.target.value)}
            placeholder={`Amount in ${assetSymbol}`}
            className="rounded-xl border border-white/10 bg-[#161632] px-4 py-3 text-hero-text placeholder:text-hero-text-muted"
          />
          <input
            type="text"
            value={receiver}
            onChange={(event) => setReceiver(event.target.value as Address | "")}
            placeholder="Recipient address"
            className="rounded-xl border border-white/10 bg-[#161632] px-4 py-3 text-hero-text placeholder:text-hero-text-muted"
          />
          {!isConnected && <p className="text-xs text-hero-text-muted">Connect your wallet to continue.</p>}
          {message && <p className="text-xs text-hero-text-muted">{message}</p>}
          <div className="flex flex-col gap-3 sm:flex-row">
            <Button
              type="submit"
              variant="hero"
              className="rounded-full px-6 py-3 text-sm font-medium"
              disabled={!isConnected || isWithdrawPending}
            >
              {isWithdrawPending ? "Sending..." : "Confirm Send"}
            </Button>
            <Button
              type="button"
              variant="secondary"
              className="rounded-full px-6 py-3 text-sm font-medium text-hero-text"
              onClick={onClose}
            >
              Cancel
            </Button>
          </div>
        </form>
      </div>
    </div>
  );
};
